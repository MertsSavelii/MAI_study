...
namespace RaGlib {
    public class FSAutomateWithOpSymbols : FSAutomate
    {
        ...
        private Dictionary<OPSymbol, operation> OpDictionary = new Dictionary<OPSymbol, operation>();
        public List<OPSymbol> SigmaOP { set; get; } = null; // множество алфавит операционных символов

        public FSAutomateWithOpSymbols(List<Symbol> Q, List<Symbol> Sigma, List<OPSymbol> SigmaOP, List<Symbol> F, Symbol q0)
        {
            this.Q = Q;
            this.Sigma = Sigma;
            ...
            OpDictionary.Add(new OPSymbol("{save}"), OP);
            OP = new operation(Load);
            OpDictionary.Add(new OPSymbol("{load}"), OP);
            OP = new operation(Split);
            OpDictionary.Add(new OPSymbol("{split}"), OP);
        }
        ...
        {
            this.Delta.Add(new DeltaQSigma(state, term, new List<Symbol> { new Symbol(nextState), new OPSymbol(opSymbol) }));
        }
            ...
            for (; i < chineSymbol.Length; i++)
            {
                ...
                foreach (var d in this.Delta)
                {
                    if (d.LHSQ == currState && d.LHSS == chineSymbol.Substring(i, 1))
                    {
                        ...
                        {
                            operation OP = OpDictionary[new OPSymbol(d.RHSQ[1].symbol)];
