...
namespace RaGlib
{
    ...
    {
        public List<OPSymbol> Top = null;
        public GrammarWithOpSymbol(List<Symbol> T, List<OPSymbol> Top, List<Symbol> V, string S0)
        {
            this.T = T;
            this.V = V;
            this.Top = Top;
        ...
        public FSAutomateWithOpSymbols Transform()
        {
            ...
            foreach (var p in this.P)
            {
                ...
                foreach (var t in this.T)
                {
                    ...
                    {
                        if (p.RHS.Contains(t) && this.Top.Contains(new OPSymbol(p.RHS[1].ToString())))
                            KA.AddRule(p.LHS.symbol, t.symbol, OpSymbol(p.RHS), "qf");

                        else if (p.RHS.Contains(t) && !this.Top.Contains(new OPSymbol(p.RHS[1].ToString())))
        ...
        private string OpSymbol(List<Symbol> array)
        {
            ...
            foreach (var s in array)
            {
                if (this.Top.Contains(new OPSymbol(s.symbol)))
                    NoTermin = s.symbol;
            }
            return NoTermin;
        }
        private List<OPSymbol> OpSymbolReturn(List<Symbol> array)
        {
            var OpSymbol = new List<OPSymbol>();
            bool flag = true; // added
            foreach (var s in array)
                if (this.Top.Contains(new OPSymbol(s.symbol)))
                {
                    flag = false; // added
                    OpSymbol.Add(new OPSymbol(s.symbol));
                }
            if (flag)
                return null; // added
            else
