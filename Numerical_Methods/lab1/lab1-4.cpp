...
double rmsNonDiagonal(const vector<vector<double>>& matrix) {
    double sum = 0.0;
    int n = matrix.size();
    int count = 0;
    for (int i = 0; i < n; i++)
...
pair<int, int> maxUpDiagonalElement(const vector<vector<double>>& A) {
...
{
    int im = maxUpDiagonalElement(A).first;
    int jm = maxUpDiagonalElement(A).second;
